    ...
    try {
      ...
      if (!storedUsers) {
        ...
        const migratedUsers = parsedUsers.map((u: NewUser) => ({
          ...u,
          roles: u.roles?.filter(r => r === 'booking') || (u.role === 'admin' ? ['booking'] : [])
        }));
      ...
      if (storedAuth) {
        ...
        parsedUser.roles = parsedUser.roles?.filter((r: string) => r === 'booking');
        setUser(parsedUser);
      }
    } catch (error) {
  ...
  const login = useCallback((username: string, password: string): User | null => {
    ...
    if (userToLogin && userToLogin.password === password) {
      const loggedInUser: User = { 
        ...
        roles: userToLogin.roles?.filter((r:string) => r === 'booking') || [],
