...
export const waybillSchema = z.object({
  ...
  senderAddress: z.string({ required_error: "Sender address is required."}).min(10, { message: 'Please enter a valid sender address.' }).optional().or(z.literal('')),
  senderCity: z.string({ required_error: "Sender city is required."}).min(2, { message: 'Sender city must be at least 2 characters.' }).optional().or(z.literal('')),
  ...
  receiverAddress: z.string({ required_error: "Receiver address is required."}).min(10, { message: 'Please enter a valid receiver address.' }).optional().or(z.literal('')),
  receiverCity: z.string({ required_error: "Receiver city is required."}).min(2, { message: 'Receiver city must be at least 2 characters.' }).optional().or(z.literal('')),
